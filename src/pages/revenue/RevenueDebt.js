...
import { actionDeleteRevenue, actionGetRevenueDebts } from "./RevenueAction";
import { TableCellActions, HeaderAction } from "../../components";
import "./Revenue.scss";
import { REVENUE_STATUS } from "../../utils/constants/config";
...
const RevenueDebt = (props) => {
  const {
    profile = {},
    actionGetRevenueDebts,
    revenues = {},
    isFetching,
    history,
  } = props;
  ...
  const handleFetchRevenue = (params = {}) => {
    actionGetRevenueDebts({ ...params, page: params.page - 1 });
  };
    ...
    (type, value) => {
      ...
      if (!value) {
        ...
        params = {
          ...params,
          page: 1,
          query: `${type}=="${value}"`.trim() + (status ? `;${status}` : "")+ (";debit>0"),
        };
      }
  ...
  return (
    <div className="revenue-page common-page">
      <div className="revenue-content">
        <PageHeader pageTitle="Danh sách ghi nợ" />
        <HeaderAction
...
export default connect(
  (state) => ({
    ...
    revenues: state.revenue?.revenueDebts,
    isFetching: state.revenue?.isFetching,
  }),
  { actionGetRevenueDebts }
)(withRouter(RevenueDebt));
